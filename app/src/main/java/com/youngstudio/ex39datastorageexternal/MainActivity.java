package com.youngstudio.ex39datastorageexternal;
...
public class MainActivity extends AppCompatActivity {
    ...
    //requestPermissions()메소드를 실행해서 보여진 퍼미션요청 다이얼로그의
    //DENY,ALLOW 버튼 중 하나를 선택하면 자동으로 실행되는 콜백메소드
    ...
    public void clickBtn(View view) {
        ...
        String state= Environment.getExternalStorageState();
        if( !state.equals(Environment.MEDIA_MOUNTED)){
            Toast.makeText(this, "외부저장소가 없다", Toast.LENGTH_SHORT).show();
            return;
        };
        ...
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.M){
            ...
            int checkResult= checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE); //위험한것들만 동적퍼미션필요(카메라,위치) WRITE쓰면 READ도 자동으로 됨

            //퍼미션이 허용되어 있지 않다면?
            if(checkResult == PackageManager.PERMISSION_DENIED){
                //사용자에게 퍼미션을 요청하는 다이얼로그를 보여주는 메소드를 실행 (안드로이드가 미리 정해놓음)
        ...
        File path=Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS); //다운로드경로얻어오기
        if(path!=null)tv.setText(path.getPath());

        File file= new File(path,"aaa.txt");//경로+파일명결합
        try {
            FileWriter fw= new FileWriter(file,true);//파일아웃풋스트림 덮어쓰기방지로 true
